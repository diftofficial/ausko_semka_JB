...
namespace ds::adt {
    ...
    {
        // 1) Je to presne ten istý objekt?
        if (this == &other) {
            return true;
        }

        // 2) Skús pretypovať na HashTable<K,T>
        auto pOther = dynamic_cast<const HashTable<K, T>*>(&other);
        if (!pOther) {
            // iný typ tabuľky → nemôžu byť rovnaké
            return false;
        }
        const HashTable<K, T>& o = *pOther;

        // 3) Veľkosti musia súhlasiť
        if (this->size() != o.size()) {
            return false;
        }

        // 4) Pre každý prvok v druhej tabuľke
        for (auto it = o.begin(); it != o.end(); ++it) {
            const auto& item = *it;       // TableItem<K,T>&
            const K& key = item.key_;
            const T& valO = item.data_;

            // 5) Skús nájsť rovnaký kľúč v tejto tabuľke
            T* valThisPtr = nullptr;
            if (!this->tryFind(key, valThisPtr)) {
                return false;           // kľúč vôbec neexistuje
            }

            // 6) Porovnaj uložené dáta
            if (!(valO == *valThisPtr)) {
                return false;           // rovnaký kľúč, ale iné dáta
            }
        }

        // 7) Všetky porovnania prešli
        return true;
    }
    ...
    {
        size_t idx = hashFunction_(key) % CAPACITY;
        auto* block = primaryRegion_->access(idx);
        auto*& bucket = block->data_;
        // len keď bucket ešte neexistuje, vytvoríme ho
        if (!bucket) {
            bucket = new SynonymTable();
        }

        // ak kľúč už v bucket-e je → vyhoďme výnimku
        T* existing = nullptr;
        if (bucket->tryFind(key, existing)) {
            throw std::runtime_error("HashTable::insert: duplicate key");
        }
        // inak normálne vložíme a zvýšime size_
    ...
    {
        size_t idx = hashFunction_(key) % CAPACITY;
        auto* bucket = primaryRegion_->access(idx)->data_;
    ...
    T HashTable<K, T>::remove(const K& key)
    {
        ...
        // delegate removal to the synonym table
        T value = bucket->remove(key);
        --size_;

        // if bucket is now empty, free it
        if (bucket->isEmpty()) {
            delete bucket;
            block->data_ = nullptr;
        }
        ...
        HashTable<K, T>::HashTableIterator::operator++()
    {
        ...
        if (synonymIterator_) {
            ++(*synonymIterator_);
            // if we reached the end of this chain, move to the next non‐empty bucket
            if (*synonymIterator_ == (**tablesCurrent_)->end()) {
                // clean up old iterator
                delete synonymIterator_;
                synonymIterator_ = nullptr;

                // skip ahead to next non‐null bucket
                do {
                    ++(*tablesCurrent_);
                } while (*tablesCurrent_ != *tablesLast_ && **tablesCurrent_ == nullptr);

                // if there's still a bucket, start at its begin()
